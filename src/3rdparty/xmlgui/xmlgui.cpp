...
{  
	...
	if(element->Attribute("basecolor"))
	{
		...
		widget->setBaseColor(strToColour(element->Attribute("basecolor")->c_str()));
	}
	...
	if(element->Attribute("foregroundcolor"))
	{
		...
		widget->setForegroundColor(strToColour(element->Attribute("foregroundcolor")->c_str()));
	}
	...
	if(element->Attribute("backgroundcolor"))
	{
		...
		widget->setBackgroundColor(strToColour(element->Attribute("backgroundcolor")->c_str()));
	}
...
void XmlGui::parseTextField(TiXmlElement *element,gcn::Widget *parent)
{
	...
	gcn::TextField *textfield = new gcn::TextField(""); // without "" widget is not visible
...
{
    // Edited by Miroslav Sobotka
    if( (unsigned)i >= items.size() ) // guichan hack, because he needed *sometimes* element at "size"
    {
        return std::string("");
    }
    else
